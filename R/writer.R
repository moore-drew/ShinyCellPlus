...
wrSVmarkersAll <- function(prefix, markers.all) {
  ...
  if(markers.all == TRUE) {
    ...
    glue::glue(
            ...
            '   }}) \n',
            '   \n',
            '   output${prefix}m_all.ui <- renderDataTable ( \n',
...
wrSVgeneSig <- function(prefix, gene.ranks) {
  ...
  glue::glue(
            ...
            '   if(class({prefix}gene_ranks) == "matrix") {{ # flimsy check, need to find better way \n',
...
wrSVvolc <- function(prefix, volc.plot) {
  ...
  glue::glue(
            ...
            '    }} \n',
            '    \n',  
            '    ggOut <- ggvolc(de_genes, de_genes_selection, fc=fold_change, p_value=p_value, size_var="set_size") + labs(title=de_select) + xlim(xlim_down, xlim_up) + ylim(0, ylim) \n',
            '    \n',  
            '    return(ggOut) \n',
            '  }} \n',
            '  \n',
            ...
            '    if({prefix}de_genes_ggvolc[1,1] == "ERROR") {{ \n',
            '      ggplot() + labs(title="ERROR: data file \'{prefix}de_genes_ggvolc.rds\' not found, cannot create plots!") \n',
            '    }} \n',
            '    else {{ \n',
            '      scVolc({prefix}de_genes_ggvolc, input$volc_de_select, de_genes_selection=NULL, input$volc_subset2, \n',
            ...
            '      plot = scVolc({prefix}de_genes_ggvolc, input$volc_de_select, de_genes_selection=NULL, input$volc_subset2, \n',
            '             input$volc_fold_change, input$volc_p_value, input$volc_subset_toggle, input$volc_top10_toggle, input$volc_xlim_down, input$volc_xlim_up, input$volc_ylim) \n',
            '      ) \n',
            '    }} \n',
            '  ) \n',
...
wrUImarkersAll <- function(prefix, markers.all) {
  ...
  if(markers.all == TRUE) {
    graphs <- graphs + glue::glue(
        ...
        '    br(),br(), \n',
        '    fluidRow( \n',
        '      selectInput("{prefix}m_all_select", "Select cluster:", choices=strsplit({prefix}conf[grp==TRUE][UI=="seurat_clusters"]$fID, "\\\\|")[[1]]), # would you ever want to look at all? \n',
        '      column(12, \n',
...
wrUIvolc <- function(prefix, volc.plot) {
  ...
  if(volc.plot == TRUE) {
    graphs <- graphs + glue::glue(
      ...
      '        br(), \n',
      '        sliderInput("volc_fold_change", "Fold Change", 0, 1, 0.3, round=-1, step=0.1), # dynamic max? \n',
      '        textInput("volc_p_value", "Adjusted p-Value", placeholder="0.05"), \n',
      '        sliderInput("volc_xlim_up", "Change upperbound range of foldchange", 0, 10, 3, round=-1, step=0.1), \n',
      '        sliderInput("volc_xlim_down", "Change lowerbound range of foldchange", -10, 0, -3, round=-1, step=0.1), \n',
      '        sliderInput("volc_ylim", "Change ceiling of p-value", 30, 600, 300, round=1, step=1) \n',
      '      ), \n',
      '      column(9, \n',
