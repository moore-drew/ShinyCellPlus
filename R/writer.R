...
wrSVvolc <- function(prefix, volc.plot) {
  ...
  glue::glue(
            ...
            '    }} \n',
            '    \n',
            '    ggOut <- ggvolc(genes, genes_selection, fc=fold_change, p_value=p_value, size_var="set_size", not_sig_color=not_sig_color, down_reg_color=down_col, up_reg_color=up_col) + labs(title=de_selection) + xlim(xlim_down, xlim_up) + ylim(0, ylim) \n',
            '    \n',
            '    return(ggOut) \n',
            '  }} \n',  
            '  \n',
            '  output$volc_subset.ui <- renderUI({{ \n', 
            '    subset = levels({prefix}de_genes_ggvolc[[input$volc_subset]]) # should you find a way to encode this into the {prefix}conf file?\n',
            '    checkboxGroupInput("volc_subset2", "Select which subsets to show", inline = TRUE, \n',
            '                       choices = subset, selected = subset) \n',
            '  }}) \n', 
            '  \n',
            '  observeEvent(input$volc_subset_sel_all, {{ \n', 
            '    subset = levels({prefix}de_genes_ggvolc[[input$volc_subset]]) \n',
...
wrSVgeneOnt <- function(prefix, gene.ont) {
  if(gene.ont == TRUE) {
    glue::glue(
              ...
              '      \n',
              '      if(length(clusters_select) <= 0 ) {{ \n',
              '        return(ggplot()) \n',
              '      }} \n',
              '      else if(length(clusters_select) == 1) {{ \n',
              '        if( nrow(p$data) <= 0) {{ \n',
              '          return(ggplot()) \n',
              '        }} \n',
              '      }} \n',
              ...
              '          if( nrow(p[[category_select]][[i]]$data) <= 0 ) {{ \n',
              '            cat("\\nno data in cluster", as.character(i), ", removing it from graph object...\\n") \n',
              '            clusters_select <- clusters_select[!clusters_select %in% i] # does this mess with the selection of i in for()? \n',
              '            p[[category_select]][[i]] <- NULL \n',
              '          }} \n',
              '        }} \n',
              '      }} \n',
              '      \n',
